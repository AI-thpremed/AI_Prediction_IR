...
def train(model: torch.nn.Module, epochs: int, task: Literal['regression', 'classification'],
    ...
    for epoch in tqdm(range(epochs), desc='Epochs'):
        ...
        for batch_idx, (categorical_data, continuous_data, target,_) in train_loader_tqdm:
        ...
        with torch.no_grad():
                ...
                for _, (categorical_data, continuous_data, target,_) in enumerate(val_loader):
                    categorical_data = categorical_data.to(device)
                    continuous_data = continuous_data.to(device)
                    if task == 'regression':
...
def inference(model: torch.nn.Module, test_loader: DataLoader, task: Literal['regression', 'classification']='regression') -> np.ndarray:
    ...
    predictions = []
    with torch.no_grad():
        for _, (categorical_data, continuous_data, target,target_key) in enumerate(test_loader):
            categorical_data = categorical_data.to(device)
            continuous_data = continuous_data.to(device)
            output = model(categorical_data, continuous_data)

            predictions.extend(output.cpu().numpy().flatten())  # 使用 flatten() 以确保是1D数组
            y_true.extend(target.cpu().numpy())
            keyid.extend(target_key.cpu().numpy())
    ...
    df.to_csv('/data/gaowh/work/24process/tab-transformer/tabkanet_github_version_IR/tabkanet_nhanes_inference_results.csv', index=False)
