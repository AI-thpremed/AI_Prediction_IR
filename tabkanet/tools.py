...
def train(model: torch.nn.Module, epochs: int, task: Literal['regression', 'classification'],
    ...
    for epoch in tqdm(range(epochs), desc='Epochs'):
        ...
        with torch.no_grad():
            ...
            test_auc = roc_auc_score(y_true=y_true, y_score=predictions)
            test_conf_matrix = confusion_matrix(y_true=y_true, y_pred=y_pred)
            test_precision = precision_score(y_true=y_true, y_pred=y_pred)
            test_recall = recall_score(y_true=y_true, y_pred=y_pred)
            test_accuracy = accuracy_score(y_true=y_true, y_pred=y_pred)  # 计算准确率
            test_loss_history.append(test_loss)

        # 输出结果
        if custom_metric is not None:
            tqdm.write(f'Epoch: {epoch}, Train Loss: {loss.item():.4f}, Val Loss: {val_loss:.4f}, Val AUC: {val_auc:.4f}, Val F1: {val_metric:.4f}, Test F1: {test_metric:.4f}, Test AUC: {test_auc:.4f}, Test Precision: {test_precision:.4f}, Test Recall: {test_recall:.4f}, Test Accuracy: {test_accuracy:.4f}')
        else:
            tqdm.write(f'Epoch: {epoch}, Train Loss: {loss.item():.4f}, Val Loss: {val_loss:.4f}, Test Loss: {test_loss:.4f}, Test AUC: {test_auc:.4f}, Test Precision: {test_precision:.4f}, Test Recall: {test_recall:.4f}, Test Accuracy: {test_accuracy:.4f}')
...
def inference(model: torch.nn.Module, test_loader: DataLoader, outputpath: str,testtype:int, task: str = 'classification') -> np.ndarray:
    """
    ...
    - outputpath (str): Path to save the prediction results
    ...
    print(f'Device: {device}')  # 替换为 logging.info 如果需要
    model.to(device)
    ...
    with torch.no_grad():
        for _, (categorical_data, continuous_data, target,_) in enumerate(test_loader):
            categorical_data = categorical_data.to(device)
            continuous_data = continuous_data.to(device)
            output = model(categorical_data, continuous_data)
            y_true = np.concatenate([y_true, target.cpu().numpy()])
            probabilities = torch.sigmoid(output)
            positive_class_probabilities = probabilities[:, 1]
            predictions_auc.extend(positive_class_probabilities.tolist())

    test_auc = roc_auc_score(y_true=y_true, y_score=predictions_auc)
    print(f'Test AUC: {test_auc}')

    # 创建保存预测结果的 DataFrame
    test_df = pd.DataFrame({
        'y_true': y_true,
        'y_score': predictions_auc
    })

    # 保存 DataFrame 到 CSV 文件
    if testtype==0:
        output_file = os.path.join(outputpath, 'test_results.csv')
    else:
        output_file = os.path.join(outputpath, 'test_external_results.csv')

    test_df.to_csv(output_file, index=False)
    print(f'Predictions saved to: {output_file}')

    return np.array(predictions_auc), y_true




# def inference(model: torch.nn.Module, test_loader: DataLoader, outputpath,task: str = 'classification') -> np.ndarray:
#     """
#     Make predictions using the model
